            ...
            if is_tool_call and use_tool:
                content = {
                    ...
                    "arguments": json.dumps(
                        arguments_json if isinstance(arguments_json, dict) else arguments, ensure_ascii=False
                    ),
                }
                if function_name == "simple_browser":
                    ...
                    if match:
                        content["arguments"] = json.dumps(
                            {"query": match.group(1), "recency_days": int(match.group(2))}, ensure_ascii=False
                        )
                elif function_name == "cogview":
                    content["arguments"] = json.dumps({"prompt": arguments}, ensure_ascii=False)
...
def process_messages(messages, tools=None, tool_choice="none"):
    ...
    def filter_tools(tool_choice, tools):
        ...
        if not function_name:
            return []
        filtered_tools = [tool for tool in tools if tool.get("function", {}).get("name") == function_name]
    ...
    if isinstance(function_call, dict):
        ...
        tool_calls = [
            ChatCompletionMessageToolCall(
                id=generate_id("call_", 24), function=function_call_instance, type="function"
            )
        ]
    ...
    async for new_response in generate_stream_glm4(gen_params):
        ...
        if is_function_call:
            if not has_send_first_chunk:
                ...
                tool_call = ChatCompletionMessageToolCall(
                    index=0, id=generate_id("call_", 24), function=FunctionCall(**function_call), type="function"
                )
                message = DeltaMessage(content=None, role="assistant", function_call=None, tool_calls=[tool_call])
            ...
            delta_text = ""
            tool_call = ChatCompletionMessageToolCall(
                index=0, id=None, function=FunctionCall(**function_call), type="function"
            )
            message = DeltaMessage(content=None, role=None, function_call=None, tool_calls=[tool_call])
            choice_data = ChatCompletionResponseStreamChoice(index=0, delta=message, finish_reason=None)
            chunk = ChatCompletionResponse(
                model=model_id,
                id=response_id,
        ...
        # 常规返回
            ...
            if not has_send_first_chunk:
                ...
                choice_data = ChatCompletionResponseStreamChoice(index=0, delta=message, finish_reason=finish_reason)
                chunk = ChatCompletionResponse(
                    model=model_id,
                    id=response_id,
                    choices=[choice_data],
...
async def parse_output_text(model_id: str, value: str, function_call: ChoiceDeltaToolCallFunction = None):
    ...
    choice_data = ChatCompletionResponseStreamChoice(index=0, delta=delta, finish_reason=None)
